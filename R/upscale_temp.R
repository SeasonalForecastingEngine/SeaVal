...
#' Gridded predictions and observations constitute grid point averages, see ...
#' This means that the upscaled value assigned to a coarse grid cell should be a weighted average of the values of the fine grid cells overlapping the coarse cell, whith the weighting acounting for the
#' area of overlap. This function does this for you, as long as both the fine grid and the coarse grid are regular grids in lon/lat (consisting of lon/lat rectangles). It adresses the following challenges:
#' \itemize{
...
#' \item Grid cell area varies between latitudes, grid cells at the equator are much larger than at the poles. This affects the contribution of the fine grid cells (grid cells closer to the pole contribute less to the coarse grid cell average).
#' \item Naive merging of data tables or matching based on distance of grid cell centers frequently result in unnecessary large lookup tables that may not fit into memory when both your fine and your coarse grid are high-resolution.
#' \item Frequently, you need to upscale \emph{repeated} data between the same grid, for example when you want to upscale observations for many different times. In this case the matching between the grids should only be done once,
#' and not be repeated every time.
#' \item Even though the grid
#' }
...
#' than the fine grid.
#'
#' Even though the grids are assumed to be regular, the function allows for missing data in the form of missing grid points in dt, so you don't have to 'sqarify' it before upscaling. In fact, the function should be slightly
#' faster when missing-data-grid points are removed from dt before, since it needs to match fewer grid points.
#'
#'
#'
...
#' @param uscols column names of the data you want to upscale (can take multiple columns at once)
#' @param bycols column names for grouping if you have repeated data on the same grid, e.g. use bycols = 'date' if your data table contains observations for many dates on the same grid (and the column specifying the date is in fact called 'date').
                                   ...
                                   )
{
  ...
  d_lats = fg_lats[2:length(fg_lons)] - fg_lats[1:(length(fg_lons)-1)]
  ...
  D_lat = min(cg_lats[2:length(cg_lons)] - cg_lats[1:(length(cg_lons) - 1)])
...
#   }
  ...
  setnames(temp,c('cg_lon','cg_lat'),c('lon','lat'))
  return(temp)
}
