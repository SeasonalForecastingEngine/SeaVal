                                        ...
                                        timeout_limit)
{
  ...
  for(yy in years)
  {
    for(mm in months)
    {
      ...
      res = try(download.file(filestr, destfile = paste0(save_dir,yy,'_',mm,'.nc'), method = "auto",
...
{
  ch_dir = paste0(chirps_dir(),'monthly/')
  ...
  {
    ff = fns[i]
    dt_temp = netcdf_to_dt(paste0(ch_dir,ff),verbose = 2)
