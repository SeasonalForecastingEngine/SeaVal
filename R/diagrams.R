                    ...
                    dim.check = TRUE)
{
  ...
  if(is.null(by))
  {
    ...
    if(check_discrete_probs)
    {
      ...
      rps1 = round_probs(dt[,get(f[3])],binwidth = bindwidth)
      rps2 = round_probs(dt[,get(f[2])],binwidth = bindwidth)
      rps3 = round_probs(dt[,get(f[1])],binwidth = bindwidth)
    }
...
roc_curve_vec = function(probs,obs,interpolate = TRUE)
{
  ...
  if(interpolate)
  {
    ...
    # highlight discretization if it's at a 4%-resolution or coarser:
    if(plot_dt[,.N] <= 26)
    {
