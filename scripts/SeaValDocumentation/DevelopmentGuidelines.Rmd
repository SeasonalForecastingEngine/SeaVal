...
* Generally data tables will contain columns containing *coordinates* (e.g. `lon`/`lat`/`month`/`year`) and columns specifying *values* (e.g. `temp`,`pred`,`climatology`). Besides the usual candidates, the following columns are also considered *coordinates* rather than *values*: `lead_time`, `member` (for ensemble forecasts), `system` (when you consider different NWPs in the same data table). Per unique coordinate the data table contains only one row. 
...
The grouping variables contain coordinates for stratification. For each level of these coordinates the evaluation metric is evaluated separately and a separate result is returned. The pooling variables contain coordinates that are pooled together in the calculation. Say, for example, we want to calculate (mean) squared errors from the following typical data table:
...
You can also select all four coordinates (lon/lat/year/month) as grouping variables (resulting in one squared error per coordinate, no mean is taken) or all four coordinates as pooling variables (resulting in one number, the overall MSE). *At least in theory you should be able to do that. As of today, some of the functions might crash because I haven't paid extensive attention to that.* 

On first glance you might think 'Why not calculating on the highest resolution (i.e. grouped by all coordinates) and letting the user taking averages thereafter themselves? Then they can group themselves however they want.' This would work in the example of the MSE, but wouldn't in most other cases. You could not calculate, say, a correlation or a skill score and then average afterwards. Also, the philosophy of having grouping coordinates and pooling coordinates also applies to diagrams: You could derive reliability diagrams from a data table similar as above, grouped by month and pooling by lon/lat/year. 
...
The code example for calculating `mse_dt` above highlights another weird conundrum when working with data tables: For many (if not most) evaluation metrics, the `pool` variable(s) does not actually occur in the function code. Data table groups by `by` and takes the average error over all rows of `dt` that have identical values in the `by`-columns. This means our scores implicitly assumed that the columns in `by` and `pool` together contain all coordinates. Whether this is satisfied is checked by the function when `dim.check = TRUE` (the default). Most of these functions really only need the `pool` argument to be able to check whether the dimensions fit, and don't require it when you set `dim.check = FALSE`. Both the `by` and `pool` arguments should have sensible defaults, such that the user usually does not normally need to provide them. If they want to do non-standard stuff, I think it's fair to ask for extra-input to make sure that the correct thing is returned. It happened to me more than once that I accidentally averaged scores over a column I didn't mean to average over and had a hard time finding out what's wrong.

In general, different forecast/observation formats automatically require different data table formats. For example, a score applicable to ensemble forecasts might have an additional column `'member'` that does not belong to either of the four categories: If you consider our MSE-example, and you put the `'member'` column to the grouping variables you get the MSE for each member separately (but still averaged over all years) which is useless for an ensemble that is exchangable. If you put it to the pooling variables you end up with the squared error averaged over all ensemble members which is also not what you want. On the other hand, tercile forecasts, for example, do not contain a special `member`-column, but contain multiple predictions per observation (for the low-, medium- and high category). So evaluation functions for these different types of forecast may require input data in different formats and require different arguments.
...
Why is it important to specify such categories? It helps us setting defaults, testing input and have general auxiliary functions. One example of each, from the scores-for-ensemble-forecasts-category: 

* All functions in there have the default `by = by_cols_ens_fc_score()`, which is just an auxiliary function returning the typical names of grouping variables in `dt` for these kind of forecasts. If we need to change the default (e.g. add 'MONTH' because some people for some reason use capitalized column names), we can do this within the function and it automatically affects the defaults for all scores of ensemble forecasts.
...
* A general auxiliary function for this category is the function `climatology_ens_forecast` which is called by the skill scores `MSES` and `CRPSS`. This computes a leave-one-year-out climatology from the data. 
