...
On first glance one might think 'Why not simply calculating always on the highest resolution (grouped by all coordinates) and returning the full result? Then, the user can group themselves by averaging however they want.' This would work in the example of the MSE and for most other proper scores, but not for other metrics. You could not calculate, say, a correlation or a skill score separately for each coordinate (i.e. based on a single value), and then average afterwards. Also, the philosophy of having grouping coordinates and pooling coordinates also applies to diagrams: You could derive reliability diagrams from a data table similar as above, grouped by month and pooling by lon/lat/year. 
